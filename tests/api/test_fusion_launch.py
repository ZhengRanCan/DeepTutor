...
def test_wrong_audience_rejects_code():
    assert exchange_launch_code(issue_launch_code("learner-1"), "wrong", "lesson-1") is None
...
def test_expired_code_and_session_binding_reject(monkeypatch):
    code = issue_launch_code("learner-1")
    monkeypatch.setattr(delegation.time, "time", lambda: 9_999_999_999)
    assert exchange_launch_code(code, "openmaic", "lesson-1") is None


def test_exchange_has_exact_minimum_scope_and_session_binding():
    result = exchange_launch_code(issue_launch_code("learner-1"), "openmaic", "lesson-1")
    assert (
        result
        and tuple(result["scope"]) == SCOPES
        and result["lessonSessionId"] == "lesson-1"
        and result["audience"] == "openmaic"
    )
...
def test_allowlist_authorizes_only_explicit_test_user(monkeypatch):
    class Registry:
        def select_for_launch(self, learner_id, requested):
            return {"courseScopeId": "course-1", "revision": "r1"}

    monkeypatch.setattr(
        "deeptutor.api.routers.fusion_launch.get_course_scope_registry", lambda: Registry()
    )
    monkeypatch.setenv("ENVIRONMENT", "test")
    monkeypatch.setenv("FUSION_TEST_USER_ALLOWLIST", "learner-ok")
    client = launch_client(TokenPayload(username="ok", role="user", user_id="learner-ok"))
    assert client.post("/api/v1/fusion/launch-codes").status_code == 200
    client = launch_client(TokenPayload(username="no", role="user", user_id="learner-no"))
    assert client.post("/api/v1/fusion/launch-codes").status_code == 403
...
def test_production_auth_disabled_rejects_issue(monkeypatch):
    ...
    monkeypatch.setenv("FUSION_DEVELOPMENT_LEARNER_ID", "demo")
    assert launch_client(None).post("/api/v1/fusion/launch-codes").status_code == 403
